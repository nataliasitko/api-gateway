...
* You have at least two deployed workloads.
...
1. Go to **Discovery and Network > APIRules** and select **Create**. 
2. Provide the APIRule's name.
3. In the Gateway section, provide the Gateway's namespace and name. If you use a Kyma domain, select the `kyma-system` namespace and `kyma-gateway` as the Gateway's name.
   1. The Gateway's namespace. If you use a Kyma domain, select the `kyma-system` namespace.
   2. The Gateway's name. If you use a Kyma domain, select `kyma-gateway`. 
4. In the **Host** field, enter `{SUBDOMAIN}.{DOMAIN_TO_EXPORT_WORKLOADS}`. `{DOMAIN_TO_EXPORT_WORKLOADS}` is the name of your domain. `{SUBDOMAIN}` is a subdomain's name of your choice.
5. 
